...
- **Stale KEK detection during save**: `useSaveField` triggers `lockVault()` if it encounters a `DecryptionError`. This happens when the field key was rotated (e.g., password changed) on another device, making the current session's KEK stale and incapable of decrypting/encrypting the field.
