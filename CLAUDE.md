...
- **Crypto placeholder**: `derive-placeholder.ts` uses SHA-256 — still actively imported by `auth-credentials.ts` as a stand-in until the real Argon2id flow is wired in (Step 17+)
